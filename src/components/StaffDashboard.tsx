    ...
    <div className="space-y-4 md:space-y-8">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-3">
        ...
        <div className="flex gap-2 overflow-x-auto pb-1 md:pb-0 no-scrollbar">
          <Button variant="ghost" onClick={() => setIsBulkUploading(true)} icon={Plus} className="whitespace-nowrap text-[10px] py-1.5 h-auto">
            Bulk Upload
          </Button>
          <Button variant="ghost" onClick={exportToExcel} icon={Save} className="whitespace-nowrap text-[10px] py-1.5 h-auto">
            Export
          </Button>
          <Button variant="primary" onClick={() => setActiveTab('members')} icon={Users} className="whitespace-nowrap text-[10px] py-1.5 h-auto">
            Members
          </Button>
          <Button variant="ghost" onClick={() => setActiveTab('workouts')} icon={Dumbbell} className="whitespace-nowrap text-[10px] py-1.5 h-auto">
            Workouts
          </Button>
        </div>
      </div>
        ...
        <div className="lg:col-span-2 space-y-4">
          <Card className="space-y-3 md:space-y-4">
            ...
            <div className="space-y-2 max-h-[500px] overflow-y-auto pr-1 custom-scrollbar">
              {checkins.map(ci => (
                <div key={ci.id} className="p-2 md:p-4 bg-white/5 border border-white/10 rounded-xl flex items-center justify-between group hover:border-[var(--primary)]/30 transition-all">
                  <div className="flex items-center gap-2 md:gap-4">
                    <div className="w-7 h-7 md:w-10 md:h-10 rounded-full bg-zinc-800 flex items-center justify-center font-bold text-[var(--primary)] text-[10px] md:text-base">
                      {ci.memberName[0]}
                    </div>
                    <div>
                      <p className="font-bold text-[10px] md:text-sm">{ci.memberName}</p>
                      <p className="text-[8px] md:text-xs opacity-70">{ci.day ? `${ci.day}, ${ci.time}` : new Date(ci.timestamp).toLocaleString()}</p>
                    </div>
                  </div>
                  <div className="text-right">
                    <span className={cn(
                      "text-[8px] md:text-[10px] px-2 py-0.5 rounded-full uppercase font-bold",
                    ...
                    <p className="text-[7px] md:text-[9px] font-mono opacity-30 mt-1">ID: {ci.memberId}</p>
                  </div>
                </div>
              ))}
              {checkins.length === 0 && (
