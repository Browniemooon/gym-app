...
export const MemberDashboard = ({ user, activeTab, setActiveTab }: MemberDashboardProps) => {
  ...
  // Apply Theme from Gym Details
  useEffect(() => {
    if (gymDetails?.themeColor) {
      document.documentElement.style.setProperty('--primary', gymDetails.themeColor);
    }
    return () => {
    ...
    <div className="space-y-4 md:space-y-8 pb-20 md:pb-0">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-4 md:mb-8">
        <div className="flex items-center gap-3 md:gap-4">
          {gymDetails?.logoUrl ? (
            <img src={gymDetails.logoUrl} alt="Logo" className="w-10 h-10 md:w-16 md:h-16 rounded-xl object-cover border border-white/10" referrerPolicy="no-referrer" />
          ) : (
